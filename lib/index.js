 ...
 *    - `cache` (Boolean) Wether to cache the response from proxy or not (default: `true`).
 *
...
module.exports = function GitHubCalendar (container, username, options) {
    ...
    options.proxy = options.proxy || (username => {
        return fetch(options.proxy(`http://api.bloggify.net/gh-calendar/?username=${username}`)).then(r => r.text())
    })

    let fetchCalendar = () => options.proxy(username).then(body => {
